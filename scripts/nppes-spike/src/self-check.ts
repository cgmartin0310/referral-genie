...
async function checkFixture(): Promise<void> {
  ...
  assert.equal(report.byGroup.physician_assistant, 1);
  assert.equal(report.byGroup.neurology, 1);
  assert.equal(report.byGroup.early_intervention, 1);
  assert.equal(report.addressQuality.missingAddress, 2);
  assert.equal(report.addressQuality.missingPhone, 3);
