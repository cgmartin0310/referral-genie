...
import { AddMissingDialog, TierButtons } from './Scoring';
...
export interface SourcesResponse {
  ...
  /** How many practices in this view carry each of the viewer's scores. */
...
export function disciplineText(rows: { label: string; low: number; high: number }[] | undefined): string {
  return (rows ?? [])
...
function SourceRow({
  ...
  canEdit: boolean;
  /** Score it for the whole company; null clears the score. */
  onScore: (tier: Tier | null) => void;
  scoring: boolean;
}) {
        ...
        <div className="col-span-12 flex items-center gap-3 lg:col-span-4">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-gray-900">
              ...
              {source.ownPractice && (
                <span className="ml-2 rounded bg-gray-100 px-1.5 py-0.5 text-[11px] font-medium text-gray-600" title="Added by your organization; only you see it">
                  added by you
                </span>
              )}
            </p>
            ...
            <div className="mt-1.5">
              <TierButtons value={source.score} onChange={onScore} disabled={scoring} />
            </div>
          </div>
        </div>
...
export function SourceList({ clinics, onPull, canEdit }: { clinics: Clinic[]; onPull: () => void; canEdit: boolean }) {
  const queryClient = useQueryClient();
  ...
  const [tier, setTier] = useState<Tier | 'unscored' | ''>('');
  ...
  useEffect(() => {
    ...
    const fromClinic = search.get('clinic');
    if (fromClinic) {
      setClinicId(fromClinic);
      setHideListed(true);
    }
    const wanted = search.get('tier');
    if (wanted === 'unscored' || (TIERS as readonly string[]).includes(wanted ?? '')) setTier(wanted as Tier | 'unscored');
  }, []);
  ...
  }, [q, countyFips, hasFax, hideListed, clinicId, tier]);
  ...
  // Score for the whole company. The row shows the new score at once; the list refreshes after.
  const score = async (ids: string[], next: Tier | null) => {
    setScoring((list) => [...list, ...ids]);
    queryClient.setQueryData<SourcesResponse>(['practices', params], (current) =>
      current ? { ...current, practices: current.practices.map((row) => (ids.includes(row.id) ? { ...row, score: next } : row)) } : current,
    );
    try {
      await axios.patch('/api/practice-scores', { practiceIds: ids, tier: next });
      if (ids.length > 1) {
        toast.success(`${ids.length} practices scored ${next ? TIER_INFO[next].label : 'as not scored'}.`);
        setSelected(new Set());
      }
    } catch (error) {
      toast.error((axios.isAxiosError(error) ? error.response?.data?.error : null) || 'The score was not saved');
    } finally {
      setScoring((list) => list.filter((id) => !ids.includes(id)));
      queryClient.invalidateQueries({ queryKey: ['practices'] });
  ...
  const tierChips: { key: Tier | 'unscored' | ''; label: string }[] = [
    { key: '', label: 'All' },
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {tierChips.map((chip) => (
          <button
            ...
            title={chip.key && chip.key !== 'unscored' ? TIER_INFO[chip.key].outreach : undefined}
            className={classNames(
              'rounded-full px-3 py-1 text-xs font-medium ring-1 ring-inset',
              tier === chip.key ? 'bg-gray-900 text-white ring-gray-900' : 'bg-white text-gray-700 ring-gray-300 hover:bg-gray-50',
            )}
          >
            {chip.label}
            {chip.key !== '' && <span className="ml-1 opacity-70">{tierCounts[chip.key] ?? 0}</span>}
          </button>
        ))}
        {selected.size > 0 && (
          <span className="ml-2 flex items-center gap-2 rounded-md bg-gray-900 px-3 py-1 text-xs text-white">
            Score {selected.size} selected:
            <TierButtons value={null} onChange={(next) => next && score([...selected], next)} disabled={scoring.length > 0} />
          </span>
        )}
        {clinics.length > 0 && (
          <button
            ...
            className="ml-auto inline-flex items-center gap-1 rounded-md bg-white px-3 py-1.5 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50"
          >
            <PlusIcon className="h-4 w-4" /> Add a missing practice
          </button>
        )}
      </div>
      <AddMissingDialog
          ...
          <div className="px-6 py-12 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {canEdit
                ...
                : 'Nothing here yet. Pick the counties your locations serve under Our Clinics; their practices appear here to score.'}
            </p>
            {canEdit && (
              <button
                type="button"
          ...
          <ul role="list" className="divide-y divide-gray-200">
            {sources.map((source) => (
              <SourceRow
                ...
                canEdit={canEdit || source.ownPractice}
                onScore={(next) => score([source.id], next)}
                scoring={scoring.includes(source.id)}
              />
            ))}
          </ul>
        )}
